...
while True:
    ...
    G_histogram = cv2.calcHist([frame2], [0], None, [32], [0, 256])
    B_histogram = cv2.calcHist([frame1], [0], None, [32], [0, 256])
    
    R_histogram_img=draw_histo(R_histogram)
    G_histogram_img=draw_histo(G_histogram)
    B_histogram_img=draw_histo(B_histogram)
    ...
    crop_R_histo = frame3[0:h_R_histo, 0:w_R_histo]
    ...
    #added_img=cv2.add(frame3,R_histogram_img)
    ...
    cv2.imshow('R_drow_histogram',R_histogram_img)
    #cv2.imshow('R_drow_histogram',added_img)
    cv2.imshow('B_drow_histogram',B_histogram_img)
    cv2.imshow('G_drow_histogram',G_histogram_img)
